...
public class ByteMessageDecoder implements MessageDecoder {
    ...
    @Override
    public Message decodeMessage(ByteBuf buf, ObjectModel model) {
    ...
    private Object decodeArgumentValue(ByteBuf buf, DataType dataType, LengthType lengthType) {
        switch (dataType) {
            case UNSIGNED_BYTE:
            case ENUM: {
    ...
    private String decodeString(ByteBuf buf, LengthType lengthType) {
        int contentLength;
        switch (lengthType) {
            case UNSIGNED_BYTE: {
    ...
    private int readVariantLength(ByteBuf buf) {
